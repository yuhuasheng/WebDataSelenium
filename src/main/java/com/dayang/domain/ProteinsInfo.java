package com.dayang.domain;

/**
 * describe:
 *
 * @author 230257
 * @date 2020/07/17
 */
public class ProteinsInfo {

    private String cat;

    private String productName;

    private String productOverview;

    private String description;

    private String sourceHost;

    private String species;

    private String applications;

    private String tag;

    private String form;

    private String activity;

    private String formulation;

    private String molecularMass;

    private String molecularWeight;

    private String purity;

    private String concentration;

    private String endotoxin;

    private String predictedNTerminus;

    private String bioActivity;

    private String unitDefinition;

    private String aaSequence;

    private String proteinLength;

    private String storage;

    private String reconstitution;

    private String storageBuffer;

    private String tissueSpecificity;

    private String shippingCondition;

    private String stability;

    private String usage;

    private String qualityControlTest;

    private String preservative;

    private String sequenceSimilarities;

    private String geneName;

    private String officialSymbol;

    private String synonyms;

    private String geneId;

    private String mRNARefseq;

    private String proteinRefseq;

    private String MIM;

    private String uniProtId;

    private String chromosomeLocation;

    private String function;

    private String url;

    public String getCat() {
        return cat;
    }

    public void setCat(String cat) {
        this.cat = cat;
    }

    public String getProductName() {
        return productName;
    }

    public void setProductName(String productName) {
        this.productName = productName;
    }

    public String getProductOverview() {
        return productOverview;
    }

    public void setProductOverview(String productOverview) {
        this.productOverview = productOverview;
    }

    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description;
    }

    public String getSourceHost() {
        return sourceHost;
    }

    public void setSourceHost(String sourceHost) {
        this.sourceHost = sourceHost;
    }

    public String getSpecies() {
        return species;
    }

    public void setSpecies(String species) {
        this.species = species;
    }

    public String getApplications() {
        return applications;
    }

    public void setApplications(String applications) {
        this.applications = applications;
    }

    public String getTag() {
        return tag;
    }

    public void setTag(String tag) {
        this.tag = tag;
    }

    public String getForm() {
        return form;
    }

    public void setForm(String form) {
        this.form = form;
    }

    public String getActivity() {
        return activity;
    }

    public void setActivity(String activity) {
        this.activity = activity;
    }

    public String getFormulation() {
        return formulation;
    }

    public void setFormulation(String formulation) {
        this.formulation = formulation;
    }

    public String getMolecularMass() {
        return molecularMass;
    }

    public void setMolecularMass(String molecularMass) {
        this.molecularMass = molecularMass;
    }

    public String getMolecularWeight() {
        return molecularWeight;
    }

    public void setMolecularWeight(String molecularWeight) {
        this.molecularWeight = molecularWeight;
    }

    public String getPurity() {
        return purity;
    }

    public void setPurity(String purity) {
        this.purity = purity;
    }

    public String getConcentration() {
        return concentration;
    }

    public void setConcentration(String concentration) {
        this.concentration = concentration;
    }

    public String getEndotoxin() {
        return endotoxin;
    }

    public void setEndotoxin(String endotoxin) {
        this.endotoxin = endotoxin;
    }

    public String getPredictedNTerminus() {
        return predictedNTerminus;
    }

    public void setPredictedNTerminus(String predictedNTerminus) {
        this.predictedNTerminus = predictedNTerminus;
    }

    public String getBioActivity() {
        return bioActivity;
    }

    public void setBioActivity(String bioActivity) {
        this.bioActivity = bioActivity;
    }

    public String getUnitDefinition() {
        return unitDefinition;
    }

    public void setUnitDefinition(String unitDefinition) {
        this.unitDefinition = unitDefinition;
    }

    public String getAaSequence() {
        return aaSequence;
    }

    public void setAaSequence(String aaSequence) {
        this.aaSequence = aaSequence;
    }

    public String getProteinLength() {
        return proteinLength;
    }

    public void setProteinLength(String proteinLength) {
        this.proteinLength = proteinLength;
    }

    public String getStorage() {
        return storage;
    }

    public void setStorage(String storage) {
        this.storage = storage;
    }

    public String getReconstitution() {
        return reconstitution;
    }

    public void setReconstitution(String reconstitution) {
        this.reconstitution = reconstitution;
    }

    public String getStorageBuffer() {
        return storageBuffer;
    }

    public void setStorageBuffer(String storageBuffer) {
        this.storageBuffer = storageBuffer;
    }

    public String getTissueSpecificity() {
        return tissueSpecificity;
    }

    public void setTissueSpecificity(String tissueSpecificity) {
        this.tissueSpecificity = tissueSpecificity;
    }

    public String getShippingCondition() {
        return shippingCondition;
    }

    public void setShippingCondition(String shippingCondition) {
        this.shippingCondition = shippingCondition;
    }

    public String getStability() {
        return stability;
    }

    public void setStability(String stability) {
        this.stability = stability;
    }

    public String getUsage() {
        return usage;
    }

    public void setUsage(String usage) {
        this.usage = usage;
    }

    public String getQualityControlTest() {
        return qualityControlTest;
    }

    public void setQualityControlTest(String qualityControlTest) {
        this.qualityControlTest = qualityControlTest;
    }

    public String getPreservative() {
        return preservative;
    }

    public void setPreservative(String preservative) {
        this.preservative = preservative;
    }

    public String getSequenceSimilarities() {
        return sequenceSimilarities;
    }

    public void setSequenceSimilarities(String sequenceSimilarities) {
        this.sequenceSimilarities = sequenceSimilarities;
    }

    public String getGeneName() {
        return geneName;
    }

    public void setGeneName(String geneName) {
        this.geneName = geneName;
    }

    public String getOfficialSymbol() {
        return officialSymbol;
    }

    public void setOfficialSymbol(String officialSymbol) {
        this.officialSymbol = officialSymbol;
    }

    public String getSynonyms() {
        return synonyms;
    }

    public void setSynonyms(String synonyms) {
        this.synonyms = synonyms;
    }

    public String getGeneId() {
        return geneId;
    }

    public void setGeneId(String geneId) {
        this.geneId = geneId;
    }

    public String getmRNARefseq() {
        return mRNARefseq;
    }

    public void setmRNARefseq(String mRNARefseq) {
        this.mRNARefseq = mRNARefseq;
    }

    public String getProteinRefseq() {
        return proteinRefseq;
    }

    public void setProteinRefseq(String proteinRefseq) {
        this.proteinRefseq = proteinRefseq;
    }

    public String getMIM() {
        return MIM;
    }

    public void setMIM(String MIM) {
        this.MIM = MIM;
    }

    public String getUniProtId() {
        return uniProtId;
    }

    public void setUniProtId(String uniProtId) {
        this.uniProtId = uniProtId;
    }

    public String getChromosomeLocation() {
        return chromosomeLocation;
    }

    public void setChromosomeLocation(String chromosomeLocation) {
        this.chromosomeLocation = chromosomeLocation;
    }

    public String getFunction() {
        return function;
    }

    public void setFunction(String function) {
        this.function = function;
    }

    public String getUrl() {
        return url;
    }

    public void setUrl(String url) {
        this.url = url;
    }
}
